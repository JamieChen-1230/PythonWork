...
class Pagination(object):
    ...
    自定義分頁類
    """
    def __init__(self, totalCount, currentPage, perPageItemNum=3, maxPageNum=7):
        """
    ...
    def page_str(self, base_url):
        ...
        page_list = []
        # --------首頁--------
        first = "<li><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>首頁</a></li>" % (base_url, 1)
        ...
        if self.currentPage == 1:
            ...
            prev = "<li><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>上一頁</a></li>" % (base_url, self.currentPage - 1)
        page_list.append(prev)

        # --------頁碼--------
        for i in self.page_num_range():
            if i == self.currentPage:
                temp = "<li class='active'><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>%s</a></li>" % (base_url, i, i)
            else:
                temp = "<li><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>%s</a></li>" % (base_url, i, i)
            page_list.append(temp)

        # --------下一頁--------
        if self.currentPage == self.num_pages:
            forward = "<li><a>下一頁</a></li>"
        else:
            forward = "<li><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>下一頁</a></li>" % (base_url, self.currentPage + 1)
        page_list.append(forward)

        # --------尾頁--------
        last = "<li><a onclick='ajax_change_page(this);' baseurl='%s' p='%s'>尾頁</a></li>" % (base_url, self.num_pages)
